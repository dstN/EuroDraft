...
<template>
  <div class="bezel-card animate-scale-in">
    <div class="bezel-inner p-5 sm:p-6 space-y-5">
      <!-- Live Stage Header -->
      <div class="flex items-center justify-between border-b border-zinc-200 dark:border-white/5 pb-3">
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs font-mono font-bold uppercase tracking-widest text-rose-400">
        ...
        <UButton
          ...
          class="font-mono text-xs font-semibold cursor-pointer"
      ...
      <div class="flex items-center justify-between gap-4 py-2">
        ...
        <div class="flex-1 flex flex-col items-center text-center space-y-1.5 min-w-0">
          ...
          <h3
            ...
            :class="match.teamA.team.id === playerTeamId ? 'text-emerald-400' : 'text-zinc-900 dark:text-white'"
          >
        ...
        <div class="text-center px-4 shrink-0">
          <div class="font-mono text-3xl sm:text-5xl font-black text-emerald-400 tracking-wider">
            {{ currentScoreA }} – {{ currentScoreB }}
          </div>
          <p
            v-if="isFinished && match.penalties"
            class="text-xs font-mono text-amber-400 font-bold mt-1"
          >
            ({{ match.penalties.teamA }}–{{ match.penalties.teamB }} pens)
          </p>
          <p
            v-else
            class="text-[10px] font-mono uppercase text-zinc-400 mt-1 font-semibold"
          >
        ...
        <div class="flex-1 flex flex-col items-center text-center space-y-1.5 min-w-0">
          ...
          <h3
            class="text-sm sm:text-base font-black truncate max-w-full"
            :class="match.teamB.team.id === playerTeamId ? 'text-emerald-400' : 'text-zinc-900 dark:text-white'"
          >
      ...
      <div class="space-y-2 pt-2 border-t border-zinc-200 dark:border-white/5 max-h-48 overflow-y-auto custom-scroll pr-1">
        <div
        ...
        >
          <span class="font-mono font-bold text-zinc-400 w-7 shrink-0">{{ ev.minute }}'</span>
          <span class="flex-1 leading-snug">{{ ev.description }}</span>
          <span
            v-if="ev.type === 'goal'"
            class="font-mono font-black text-[10px] px-1.5 py-0.5 rounded bg-emerald-500 text-white shrink-0"
          >
            {{ ev.scoreA }}–{{ ev.scoreB }}
          </span>
        </div>
